...
public class SampleController implements Initializable {
	...
	private Mat goodStamp, twinckeStamp, sunStamp, smileStamp, heartStamp, dogStamp, angryStamp, catStamp, pieceStamp,
			punchStamp, surpriseStamp;
	...
	public void saveImage(ActionEvent event) {
		...
		fileChooser.getExtensionFilters().addAll(new ExtensionFilter("Bitmap Image", "*.bmp"));
	...
	public void effectBarChanged(MouseEvent event) {
		...
		if (0 < value) {
			...
			switch (effect) {
			case "Sketchi":
				Photo.pencilSketch(mat, mat, mat, (float) (0.7 * (50.0 + value)), (float) (0.0007 * (50.0 + value)),
						(float) (0.001 * (150.0 - value)));
				break;
			...
			/* openCVでjar中のurlを扱えないためのファイルコピー */
			{
				URL url = getClass().getClassLoader().getResource("haarcascade_frontalcatface.xml");
				copyUrlToFile(url, "temp.xml");
				ImageProcessor.detect(mat, mat, "temp.xml", new Scalar(0, 255, 0), (int) value / 30 + 1);
	...
	public void imageSelect(ActionEvent event) {
		...
		fileChooser.getExtensionFilters()
				.addAll(new ExtensionFilter("Image Files", "*.png", "*.jpg", "*.bmp", "*.jpeg"));
	...
	public void launchClock() {
		exec.submit(() -> {
			while (true) {
				Platform.runLater(() -> clockLabel
						.setText(LocalDateTime.now().format(DateTimeFormatter.ofPattern("MM/dd HH:mm:ss"))));
				try {
					Thread.sleep(1000);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		comboBox.getItems().addAll("None", "Sketchi", "Oil paint", "Black Pencil", "Color Pencil", "Cartoon", "Glow",
				"Sepia", "Face Detecting", "Cat Detecting", "Snowstorm", "Bubble", "Stars");
		comboBox.setValue("None");

		stampTypes.getItems().addAll("None", "Smile", "Surprise", "Angry", "Sun", "Twinkle", "Heart", "Dog", "Cat",
				"Sun", "Good Sign", "Piece Sign", "Punch");
		stampTypes.setValue("None");
		try {
			/*
			 * jarで読み込むためurlを使用。 またimreadを用いるとjarから読み込むことができない。
			 */
